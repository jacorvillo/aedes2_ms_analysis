...
p2 <- ggplot(data = yearly_df, aes(x = time, y = median_data)) +
  ...
  geom_line(aes(
    ...
    linetype = paste0("Smoothed Signal w/ LOESS (7 months & 9 years)")
  ), color = "red", size = 1) +
  labs(
    y = "Yearly Global Temperature (Cº)",
    x = "Year",
    linetype = ""
  ) + # Empty legend title to remove "linetype"
  theme_minimal() +
  theme(
    plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
    axis.title = element_text(size = 14, face = "bold"),
...
var_detrended_data_data <- ncvar_def("detrended_temps", "units", list(dim_time),
  -9999,
  longname = "Detrended Temperature", prec = "double"
)
...
var_norm_data <- ncvar_def("detrended_temps", "units", list(dim_time, dim_lat, dim_lon),
  -9999,
  longname = "Detrended Temp Values (3D)", prec = "double"
)

...
# Create a dataframe with time points and temperature data
...
span_choices <- seq(1, length(median_data), by = 1)  # Try spans from 1 year up to full series
...
for (i in seq_along(span_choices)) {
  ...
  # Fit LOESS model
  model <- loess(temperature ~ time, data = time_series_df, 
  ...
  # Calculate R-squared
  ss_total <- sum((time_series_df$temperature - mean(time_series_df$temperature))^2)
...
# Try for the optimal span value found in the analysis
...
for (span in spans) {
  ...
  for (nlat in seq_along(lat)) {
    for (nlon in seq_along(lon)) {
      ...
      if (all(is.na(combined_data[, nlat, nlon]))) { # If all values are NA, set the values to NA
        ...
        components <- stl(signal, s.window = "per", t.window = span)
  ...
  toptitle_decadal <- "Variance % Explained by Remainder Variability"

  fileout_trend <- paste0("4_outputs/trend_global_time_monthly_", span, ".png")
  fileout_seasonal <- paste0("4_outputs/seasonal_global_time_monthly_", span, ".png")
  fileout_decadal <- paste0("4_outputs/decadal_global_time_monthly_", span, ".png")
